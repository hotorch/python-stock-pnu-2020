...
class DBUpdater:
    def __init__(self):
        ...
        self.conn = pymysql.connect(
            host=vs.db_host,          # localhost
            user=vs.db_user,          # root
            password=vs.db_password,
            port=3307,
            db=vs.db_name,            # investar
            charset='utf8'
        )
    ...
    def read_krx_code(self):
        ...
        krx = pd.read_html(url, converters={'종목코드': lambda x: str(x)})[0]
        krx = krx[['종목코드', '회사명']]
    ...
    def replace_into_db(self, df, num, code, company):
        ...
        :param df: 데이터프레임(주식 시세)
        :param num: 실행결과에 나올 숫자
        :param code: 종목코드
        :param company: 회사명
        """
        with self.conn.cursor() as curs:
    ...
    def execute_daily(self):
        ...
        self.update_daily_price(pages_to_fetch)  # 최초실행시 프로그램이 중단된다면 config.json 파일을 삭제해야함
        ...
        t = Timer(secs, self.execute_daily)  # 특정 시간(초)이 지난 뒤 메서드(self.execute_daily) 실행
